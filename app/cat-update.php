...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    $stmt = $pdo->prepare("UPDATE cat SET type=?,title_ru=?,title_uz=?,pId=?,slug=?,sort=? WHERE id=?");

    if(!$stmt->execute([ $type,$title_ru,$title_uz,$pId,$slug,$sort,$id  ])) {
        echo "Something went wrong. Please try again later.";
        header("location: error.php");
    } else {
        $stmt = null;
        header("location: cat-read.php?id=$id");
    }
} else {
